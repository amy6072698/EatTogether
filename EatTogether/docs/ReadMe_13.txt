...
```
[] /Models/Entities/TableEntity.cs
    屬性：
        int     Id
        string  TableName
        int     SeatCount
...
[] /Models/Entities/ReservationEntity.cs
    屬性：
        int       Id
...
[] /Models/Entities/CouponEntity.cs
    屬性：
        int       Id
        string    Name
        string    Code
...
[] /Models/Entities/MemberCouponEntity.cs
    屬性：
        int       Id
        int       MemberId
        int       CouponId
...
## 設定 EatTogetherDbContext

```
[] 新增 DbSet
    DbSet<TableEntity>        Tables
    DbSet<ReservationEntity>  Reservations
    DbSet<CouponEntity>       Coupons
    DbSet<MemberCouponEntity> MemberCoupons

[] OnModelCreating 設定 Fluent API
    Tables：
        HasIndex(t => t.TableName).IsUnique()
        Property(t => t.SeatCount).HasDefaultValue(2)
        Property(t => t.Status).HasDefaultValue(0)

    Reservations：
        HasIndex(r => r.BookingNumber).IsUnique()
        HasIndex(r => r.ReservationDate)
        Property(r => r.ReservedAt).HasDefaultValueSql("GETDATE()")
        Property(r => r.AdultsCount).HasDefaultValue(1)
        Property(r => r.ChildrenCount).HasDefaultValue(0)

    Coupons：
        HasIndex(c => c.Code).IsUnique()
        Property(c => c.ReceivedCount).HasDefaultValue(0)
        Property(c => c.MinSpend).HasDefaultValue(0)

    MemberCoupons：
        HasOne(mc => mc.Member).WithMany().HasForeignKey(mc => mc.MemberId)
        HasOne(mc => mc.Coupon).WithMany().HasForeignKey(mc => mc.CouponId)
        Property(mc => mc.IsUsed).HasDefaultValue(false)

[] 執行 Migration
    Add-Migration AddMemberDTables
    Update-Database
    確認 Migration 腳本內容正確後再執行
```
